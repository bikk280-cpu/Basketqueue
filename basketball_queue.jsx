  ...
  return (
    <div style={{ display: "flex", gap: 5, flex: 1 }}>
      <input
        ...
        style={{
          flex: 1, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(232,102,61,0.7)",
...
function QueueScreen({ initial, onReset }) {
  ...
  const [editing, setEditing] = useState(null); // null | 'A' | 'B' | 'rest' | number
  ...
  const handleWin = (winner) => {
    ...
    if (newWins >= 2) {
      ...
      const next = q.shift() ?? { name: "รอทีม...", wins: 0 };
      if (winner === "A") { setTeamA({ ...winTeam }); setTeamB(next); }
      else { setTeamB({ ...winTeam }); setTeamA(next); }
      setRest(rest);
      setQueue(q);
    }
    setEditing(null);
